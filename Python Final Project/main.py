...
# Đường dẫn tới file icon 
icon_path = os.path.join(script_dir, 'gapic1.png')
# Tạo PhotoImage từ file icon
...
def open_donation_link():
    ...
    musicvu_path = os.path.join(script_dir, "donate.mp3")
    pygame.mixer.music.load(musicvu_path)
    pygame.mixer.music.play(-1)
...
def show_instructions():
    ...
    pygame.mixer.init()
    # Load background music 
    musicvu_path = os.path.join(script_dir, "instructions.mp3")
    pygame.mixer.music.load(musicvu_path)
    pygame.mixer.music.play(-1)
    instructions_window = tk.Toplevel(root)
    instructions_window.title("Instructions")
    # Add some padding
    instructions_window.config(padx=10, pady=15)
    # Frame for text
    text_frame = tk.Frame(instructions_window)
    # Format text nicely
    instructions_text = (
        "You have to pick up 📦 containers for the customers.\n\n" 
        "In each container, there are gadgets that support studying such as: "
        "📚books, 📝pens, 📝pencils, ...\n\n"
        "The customers are UEHers (members of the University of Economics "
        "🎓 Ho Chi Minh City).\n\n"  
        "Once you have enough containers, drive your car 🚗 to the university "
        "for delivery.\n\n"
        "Before that, pass a poll by answering some questions.\n\n" 
        "Good luck to you! 👍"
    )
    # Create label
    instructions_label = tk.Label(text_frame, text=instructions_text, font=("Times", 15), justify="left")
    instructions_label.pack()
    # Add frame to window    
    text_frame.pack()
    # Styling
    instructions_window.config(bg="lightgray")



# Tao background
background_path = os.path.join(script_dir, "hinhnen.png")

#Manhinh
img = [0, 0, 0]
canvas1= tk.Canvas(master=root, width=800, height=720, background="white")
canvas1.pack()   
#Background
img[0] = ImageTk.PhotoImage(Image.open(background_path).resize((820, 680)))
backgr = canvas1.create_image(0, 0, anchor=tk.NW, image=img[0])

# Create elements in the interface 

button = tk.Button(root, text="Start         ", font=("Times", 15), command=start_game, bg="yellow")
button1 = tk.Button(root, text="Exit          ", font=("Times", 15), command=exit_game, bg="green")
button2 = tk.Button(root, text="Donation ", font=("Times", 15), command=open_donation_link, bg="pink")
button3 = tk.Button(root, text="Intructions", font=("Times", 15), command=show_instructions, bg="orange")

start_button = tk.Button(root, text="Start Game", font=("Times", 15), command=start_game, bg="yellow")
exit_button = tk.Button(root, text="Exit Game", font=("Times", 15), command=exit_game, bg="green")
donate_button = tk.Button(root, text="Donate", font=("Times", 15), command= open_donation_link, bg="pink")
instructions_button = tk.Button(root, text="Instructions", font=("Times", 15), command=show_instructions, bg="orange")

button_window = canvas1.create_window(360, 120, anchor='nw', window=button)
button1_window = canvas1.create_window(360, 180, anchor='nw', window=button1)
button2_window = canvas1.create_window(360, 240, anchor='nw', window=button2)
button3_window = canvas1.create_window(360, 300, anchor='nw', window=button3)

# Music background
musicvu_path = os.path.join(script_dir, "music.mp3")
pygame.mixer.music.load(musicvu_path)
pygame.mixer.music.play(-1)
